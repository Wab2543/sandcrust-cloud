...
const CHINA_REGIONS = [
  ...
  {
    province: '河北省',
    cities: ['石家庄市', '唐山市', '秦皇岛市', '邯郸市', '邢台市', '保定市', '张家口市', '承德市', '沧州市', '廊坊市', '衡水市', '雄安新区']
  },
  {
    province: '山西省',
  ...
  {
    province: '台湾省',
    cities: ['台北市', '高雄市', '台中市', '台南市', '基隆市', '新竹市', '嘉义市']
  },
  {
    province: '香港特别行政区',
  ...
  {
    province: '澳门特别行政区',
    cities: ['花地玛堂区', '花王堂区', '望德堂区', '大堂区', '风顺堂区', '嘉模堂区', '圣方济各堂区']
  }
];

// 初始化省份下拉框
function initProvinceSelect(selectEl) {
  if (!selectEl) return;
  selectEl.innerHTML = '<option value="">-- 选择省/自治区/直辖市 --</option>';
  CHINA_REGIONS.forEach((region, index) => {
...
// 省份变更时更新城市下拉框
function updateCitySelect(provinceSelect, citySelect, countyInput) {
  if (!citySelect) return;
  const index = provinceSelect ? provinceSelect.value : '';
  citySelect.innerHTML = '<option value="">-- 选择市/区 --</option>';
  if (countyInput) countyInput.value = '';

  if (index === '' || index === null || index === undefined) return;

  const region = CHINA_REGIONS[parseInt(index)];
  if (!region) return;

  region.cities.forEach((city, ci) => {
    const opt = document.createElement('option');
    opt.value = ci;
...
// 获取完整位置字符串
function getLocationString(provinceSelect, citySelect, countyInput) {
  const pIdx = provinceSelect ? provinceSelect.value : '';
  const cIdx = citySelect ? citySelect.value : '';
  const county = countyInput ? countyInput.value.trim() : '';

  if (pIdx === '' || pIdx === null) return '';

  const region = CHINA_REGIONS[parseInt(pIdx)];
  if (!region) return '';

  let location = region.province;

  if (cIdx !== '' && cIdx !== null) {
    const city = region.cities[parseInt(cIdx)];
